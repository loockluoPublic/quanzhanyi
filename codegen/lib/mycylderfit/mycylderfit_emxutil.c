...
void c_emxInit_pointclouds_internal_(
    c_emxArray_pointclouds_internal **pEmxArray, int numDimensions)
{
...
{
  emxInit_real_T(&pStruct->Location, 2);
  emxInit_uint8_T(&pStruct->Color, 2);
  emxInit_real_T(&pStruct->Normal, 2);
  emxInit_real_T(&pStruct->Intensity, 2);
  emxInit_real_T(&pStruct->ZLimitsInternal, 2);
  c_emxInit_pointclouds_internal_(&pStruct->PointCloudArrayData, 2);
}
...
/*
 * File trailer for mycylderfit_emxutil.c
 *
 * [EOF]
 */
