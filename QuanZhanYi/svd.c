...
void b_svd(const double A[9], double U[9], double s[3], double V[9])
{
  ...
  for (q = 0; q < 2; q++) {
    ...
    if (q + 1 <= 1) {
      ...
      if (nrm == 0.0) {
        ...
        for (kase = qp1; kase < 4; kase++) {
          c_xaxpy(2, e[kase - 1], b_A, 3 * (kase - 1) + 2, work, 2);
        }
        for (kase = qp1; kase < 4; kase++) {
          d_xaxpy(2, -e[kase - 1] / e[1], work, 2, b_A, 3 * (kase - 1) + 2);
        }
      }
      for (k = qp1; k < 4; k++) {
        Vf[k - 1] = e[k - 1];
  ...
  for (q = 1; q >= 0; q--) {
    ...
    if (b_s[q] != 0.0) {
      for (kase = qp1; kase < 4; kase++) {
        qjj = (q + 3 * (kase - 1)) + 1;
        b_xaxpy(3 - q, -(b_xdotc(3 - q, U, qq + 1, U, qjj) / U[qq]), qq + 1, U,
                qjj);
      }
      for (k = q + 1; k < 4; k++) {
        kase = (k + 3 * q) - 1;
  ...
  for (q = 2; q >= 0; q--) {
    if ((q + 1 <= 1) && (e[0] != 0.0)) {
      b_xaxpy(2, -(b_xdotc(2, Vf, 2, Vf, 5) / Vf[1]), 2, Vf, 5);
      b_xaxpy(2, -(b_xdotc(2, Vf, 2, Vf, 8) / Vf[1]), 2, Vf, 8);
    }
    Vf[3 * q] = 0.0;
    Vf[3 * q + 1] = 0.0;
    Vf[3 * q + 2] = 0.0;
 ...
 * Return Type  : void
 */
void c_svd(const emxArray_real_T *A, emxArray_real_T *U, double s_data[],
  ...
  if (A->size[0] == 0) {
    ...
    for (q = 0; q < i; q++) {
      ...
      for (jj = qp1; jj < 5; jj++) {
        ...
        if (apply_transform) {
          ...
          e_xaxpy(nmq + 1, nrm, qq, b_A, qjj + 1);
        }
        e[jj - 1] = b_A->data[qjj];
      }
      if (q + 1 <= nct) {
      ...
      if (q + 1 <= nrt) {
        ...
        if (nrm == 0.0) {
          ...
          if (q + 2 <= n) {
            ...
            for (jj = qp1; jj < 5; jj++) {
              f_xaxpy(nmq, e[jj - 1], b_A, (q + n * (jj - 1)) + 2, work, q + 2);
            }
            for (jj = qp1; jj < 5; jj++) {
              f_xaxpy(nmq, -e[jj - 1] / e[q + 1], work, q + 2, b_A,
                      (q + n * (jj - 1)) + 2);
            }
          }
        }
    ...
    for (q = 3; q >= 0; q--) {
      if ((q + 1 <= nrt) && (e[q] != 0.0)) {
        ...
        for (jj = qp1; jj < 5; jj++) {
          nmq = (q + ((jj - 1) << 2)) + 2;
          xaxpy(3 - q, -(xdotc(3 - q, V, ns, V, nmq) / V[ns - 1]), ns, V, nmq);
        }
      }
      ns = q << 2;
      V[ns] = 0.0;
 ...
 * Return Type  : void
 */
void svd(const double A[4], double *U, double *s, double V[16])
{
  double e[4];
  double b_s[2];
  double absxk;
  ...
  *U = 1.0;
  for (q = 3; q >= 0; q--) {
    if ((q + 1 <= 1) && (e[0] != 0.0)) {
      for (kase = 0; kase < 3; kase++) {
        ...
        f = V[3];
        temp = V[qp1jj + 2];
        absxk = -(((V[1] * scale + V[2] * nrm) + V[3] * temp) / V[1]);
        if (!(absxk == 0.0)) {
          scale += absxk * V[1];
          V[qp1jj] = scale;
          nrm += absxk * t;
  ...
  while ((m > 0) && (iter < 75)) {
    ...
    while (!(exitg1 || (q == 0))) {
      absxk = fabs(e[0]);
      if ((absxk <= 2.2204460492503131E-16 * (fabs(b_s[0]) + fabs(b_s[1]))) ||
    ...
    case 1:
      ...
      for (k = ii_tmp_tmp; k >= q + 1; k--) {
        xrotg(&b_s[0], &f, &t, s);
        qp1jj = (m - 1) << 2;
        temp = t * V[0] + *s * V[qp1jj];
        V[qp1jj] = t * V[qp1jj] - *s * V[0];
        V[0] = temp;
        absxk = V[qp1jj + 1];
        temp = t * V[1] + *s * absxk;
        V[qp1jj + 1] = t * absxk - *s * V[1];
        V[1] = temp;
        absxk = V[qp1jj + 2];
        temp = t * V[2] + *s * absxk;
        V[qp1jj + 2] = t * absxk - *s * V[2];
        V[2] = temp;
        absxk = V[qp1jj + 3];
        temp = t * V[3] + *s * absxk;
        V[qp1jj + 3] = t * absxk - *s * V[3];
        V[3] = temp;
      }
      break;
    case 2:
    ...
    case 3:
      ...
      t = e[0] / scale;
      temp = b_s[q] / scale;
      nrm = ((absxk + f) * (absxk - f) + t * t) / 2.0;
      t *= f;
      t *= t;
      if ((nrm != 0.0) || (t != 0.0)) {
        absxk = sqrt(nrm * nrm + t);
        if (nrm < 0.0) {
          absxk = -absxk;
        }
        absxk = t / (nrm + absxk);
      ...
      f = (temp + f) * (temp - f) + absxk;
      absxk = temp * (e[q] / scale);
      for (k = q + 1; k < 2; k++) {
        xrotg(&f, &absxk, &t, &nrm);
        f = t * b_s[0] + nrm * e[0];
        scale = t * e[0] - nrm * b_s[0];
        e[0] = scale;
        absxk = nrm * b_s[1];
        b_s[1] *= t;
        temp = t * V[0] + nrm * V[4];
        ...
        V[3] = temp;
        b_s[0] = f;
        xrotg(&b_s[0], &absxk, &t, &nrm);
        f = t * scale + nrm * b_s[1];
        b_s[1] = -nrm * scale + t * b_s[1];
        absxk = nrm * e[1];
        e[1] *= t;
      }
      e[0] = f;
