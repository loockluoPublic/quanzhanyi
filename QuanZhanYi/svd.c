  ...
  while ((m + 2 > 0) && (qq < 75)) {
    ...
    case 2: {
      ...
      for (k = ii + 1; k <= m + 2; k++) {
        ...
        rt = -sqds * b;
        e[k - 1] = b * sm;
        b_xrot(U, 3 * (k - 1) + 1, 3 * (ii - 1) + 1, sm, sqds);
      }
    } break;
    case 3: {
      double b;
    ...
    default:
      ...
      while ((ii + 1 < 3) && (b_s[ii] < b_s[qp1])) {
        ...
        b_s[ii] = b_s[qp1];
        b_s[qp1] = rt;
        b_xswap(Vf, 3 * ii + 1, 3 * (ii + 1) + 1);
        b_xswap(U, 3 * ii + 1, 3 * (ii + 1) + 1);
        ii = qp1;
        qp1++;
      }
      qq = 0;
...
void c_svd(const double A[8], double U[4], double s[2], double V[16])
{
  ...
  for (q = 3; q >= 0; q--) {
    if ((q + 1 <= 2) && (e[q] != 0.0)) {
      ...
      for (jj = qp1; jj < 5; jj++) {
        qq = (q + ((jj - 1) << 2)) + 2;
        c_xaxpy(3 - q, -(xdotc(3 - q, V, qjj, V, qq) / V[qjj - 1]), qjj, V, qq);
      }
    }
    qjj = q << 2;
    V[qjj] = 0.0;
  ...
  while ((m + 2 > 0) && (qp1 < 75)) {
    ...
    case 1:
      ...
      for (k = jj; k >= ii + 1; k--) {
        ...
        xrot(V, ((k - 1) << 2) + 1, ((m + 1) << 2) + 1, sm, sqds);
      }
      break;
    case 2:
      ztest = e[ii - 1];
 ...
 * Return Type  : void
 */
void svd(const emxArray_real_T *A, emxArray_real_T *U, double s_data[],
