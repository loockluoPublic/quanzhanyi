...
void planefit(const emxArray_real_T *Points, const emxArray_real_T *PlaneParaIn,
              ...
              emxArray_real_T *PlaneParaOut, double TrianglePoints_data[],
              int TrianglePoints_size[2])
{
  ...
  for (r2 = 0; r2 < 3; r2++) {
    for (i = 0; i < nx; i++) {
      b_pointss_data[i + b_pointss->size[0] * r2] =
  ...
  for (r2 = 0; r2 < nx; r2++) {
    pointss_data[r2] = b_pointss_data[r2];
  }
  emxFree_real_T(&b_pointss);
  nx = pointss->size[0] * 3;
  p = true;
  for (r2 = 0; r2 < nx; r2++) {
    if ((!p) || (rtIsInf(pointss_data[r2]) || rtIsNaN(pointss_data[r2]))) {
      p = false;
    }
  }
  if (p) {
  ...
  if (fabs(90.0 - 57.295779513082323 *
    ...
    x_data = b_Points->data;
    for (r2 = 0; r2 < nx; r2++) {
      x_data[r2] = Points_data[3 * r2 + 2];
    }
  ...
  x_data = PlaneParaOut->data;
  x_data[0] = 0.0;
  x_data[1] = 0.0;
  x_data[2] = 0.0;
  x_data[3] = 0.0;
  TrianglePoints_size[0] = 3;
  TrianglePoints_size[1] = 1;
  TrianglePoints_data[0] = 0.0;
  TrianglePoints_data[1] = 0.0;
  TrianglePoints_data[2] = 0.0;
  if (PlaneParaIn->size[1] == 0) {
    ...
    TrianglePoints_size[0] = 3;
    TrianglePoints_size[1] = 6;
    memcpy(&TrianglePoints_data[0], &c_xfit[0], 18U * sizeof(double));
    ...
    emxEnsureCapacity_real_T(PlaneParaOut, r2);
    x_data = PlaneParaOut->data;
    x_data[0] = PlaneParaIn_data[0];
    x_data[1] = PlaneParaIn_data[1];
    x_data[2] = PlaneParaIn_data[2];
    x_data[3] = PlaneParaIn_data[3];
    x_data[4] = a;
    x_data[5] = b;
    x_data[6] = c;
    x_data[7] = d;
    ...
    a21 = yfit[r2] / yfit[nx];
    yz_idx_1_tmp = yfit[nx + 2];
    yz_idx_1 = (bb[r2] - bb[nx] * a21) / (yfit[r2 + 2] - a21 * yz_idx_1_tmp);
    /*  The point on the intersection line */
    coefficients[0] = x_val;
    coefficients[1] = (bb[nx] - yz_idx_1 * yz_idx_1_tmp) / yfit[nx];
    ...
    for (r2 = 0; r2 < 12; r2++) {
      signed char i1;
      i1 = iv3[r2];
      f_xfit[3 * r2] = d_xfit[i1];
      f_xfit[3 * r2 + 1] = c_yfit[i1];
      f_xfit[3 * r2 + 2] = b_zfit[i1];
    }
    TrianglePoints_size[0] = 3;
    TrianglePoints_size[1] = 12;
    memcpy(&TrianglePoints_data[0], &f_xfit[0], 36U * sizeof(double));
    ...
    x_data = PlaneParaOut->data;
    for (r2 = 0; r2 < 2; r2++) {
      x_data[4 * r2] = PlaneParaIn_data[4 * r2];
      i = 4 * r2 + 1;
      x_data[i] = PlaneParaIn_data[i];
      i = 4 * r2 + 2;
      x_data[i] = PlaneParaIn_data[i];
      i = 4 * r2 + 3;
      x_data[i] = PlaneParaIn_data[i];
    ...
    CrossLine(*(double(*)[4]) & PlaneParaIn_data[0],
              *(double(*)[4]) & PlaneParaIn_data[4], x_val, coefficients,
              s_data);
    /*  找到边界，确定三角点 */
    d_GenerateTrianglePoints(*(double(*)[4]) & x_data[0], BoundPoint1,
    ...
    b_yfit[1] = PointTri[4];
    zfit[1] = PointTri[5];
    d_GenerateTrianglePoints(*(double(*)[4]) & x_data[0], BoundPoint2,
                             coefficients, s_data, PointTri);
    b_xfit[2] = PointTri[0];
    b_yfit[2] = PointTri[1];
    zfit[2] = PointTri[2];
    ...
    zfit[3] = PointTri[5];
    /* %%%%% 第2、3个面的交点 %%%% */
    CrossLine(*(double(*)[4]) & x_data[4], *(double(*)[4]) & x_data[8], x_val,
              coefficients, s_data);
    /*  找到边界，确定三角点 */
    d_GenerateTrianglePoints(*(double(*)[4]) & x_data[8], BoundPoint1,
                             coefficients, s_data, PointTri);
    b_xfit[4] = PointTri[0];
    b_yfit[4] = PointTri[1];
    zfit[4] = PointTri[2];
    ...
    b_yfit[6] = PointTri[4];
    zfit[6] = PointTri[5];
    d_GenerateTrianglePoints(*(double(*)[4]) & x_data[8], BoundPoint2,
                             coefficients, s_data, PointTri);
    b_xfit[5] = PointTri[0];
    b_yfit[5] = PointTri[1];
    zfit[5] = PointTri[2];
    ...
    TrianglePoints_size[0] = 3;
    TrianglePoints_size[1] = 18;
    memcpy(&TrianglePoints_data[0], &e_xfit[0], 54U * sizeof(double));
    ...
    zfit[5] = PointTri[2];
    /* %%%%% 第3、4个面的交点 %%%% */
    CrossLine(*(double(*)[4]) & x_data[8], *(double(*)[4]) & x_data[12], x_val,
              coefficients, s_data);
    d_GenerateTrianglePoints(*(double(*)[4]) & x_data[12], BoundPoint1,
                             coefficients, s_data, PointTri);
    b_xfit[6] = PointTri[0];
    b_yfit[6] = PointTri[1];
    zfit[6] = PointTri[2];
    d_GenerateTrianglePoints(*(double(*)[4]) & x_data[12], BoundPoint2,
                             coefficients, s_data, PointTri);
    b_xfit[7] = PointTri[0];
    b_yfit[7] = PointTri[1];
    zfit[7] = PointTri[2];
    /* %%%%% 第1、4个面的交点 %%%% */
    CrossLine(*(double(*)[4]) & x_data[0], *(double(*)[4]) & x_data[12], x_val,
              coefficients, s_data);
    d_GenerateTrianglePoints(*(double(*)[4]) & x_data[12], BoundPoint1,
                             coefficients, s_data, PointTri);
    b_xfit[1] = PointTri[0];
    ...
    zfit[3] = PointTri[2];
    TrianglePoints_size[0] = 3;
    TrianglePoints_size[1] = 24;
    for (r2 = 0; r2 < 24; r2++) {
      signed char i1;
      i1 = iv2[r2];
      TrianglePoints_data[3 * r2] = b_xfit[i1];
      TrianglePoints_data[3 * r2 + 1] = b_yfit[i1];
      TrianglePoints_data[3 * r2 + 2] = zfit[i1];
    }
  }
  emxFree_real_T(&y);
  emxFree_real_T(&x);
