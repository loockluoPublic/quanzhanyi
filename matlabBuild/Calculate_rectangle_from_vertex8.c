    ...
    emxArray_real_T *BianHao)
{
  ...
  b_planefit4(side_faces_transformed1, side_faces_transformed3,
              ...
              P_bound2, a__1, TrianglePoints4, a__2, deltTih);
  /* 调用矩形参数计算 */
  ...
  deltTib_data = b_side_faces_transformed4->data;
  loop_ub =
      side_faces_transformed4->size[0] * side_faces_transformed4->size[1] - 1;
  for (i = 0; i <= loop_ub; i++) {
    deltTib_data[i] = side_faces_transformed4_data[i];
  }
  ...
  /* 计算角度 */
  ab2 = b_norm(&deltTib_data[0]);
  theta1 = acos(dot(&deltTib_data[0], &deltTib_data[4]) / ab2 /
  ...
  theta4 = acos(dot(&deltTib_data[0], &deltTib_data[28]) / ab2 /
  ...
  for (b_i = 0; b_i < loop_ub; b_i++) {
    ab2 = fabs(Ti_data[b_i]);
    if ((double)b_i + 1.0 <= ab1) {
      XMFlagA_data[b_i] = !(ab2 < fabs(ab4));
    } else if ((double)b_i + 1.0 <= 2.0 * ab1) {
      XMFlagA_data[b_i] = !(ab2 < fabs(TiYuZhi1));
    } else if ((double)b_i + 1.0 <= 3.0 * ab1) {
      XMFlagA_data[b_i] = !(ab2 < fabs(TiYuZhi2));
    } else {
      XMFlagA_data[b_i] = !(ab2 < fabs(TiYuZhi3));
    }
  }
  /*  3、计算斜面偏移值 */
  ...
  for (nx = 0; nx < loop_ub; nx++) {
    if ((double)nx + 1.0 <= ab1) {
      if (XMFlagA_data[nx] == 1.0) {
        /* 斜面上 */
        XieMianPianYi_data[nx] = a_data[nx] * tan(theta4 - 1.5707963267948966);
      } else {
        XieMianPianYi_data[nx] = 0.0;
      }
    } else if ((double)nx + 1.0 <= 2.0 * ab1) {
      if (XMFlagA_data[nx] == 1.0) {
        /* 斜面上 */
        XieMianPianYi_data[nx] = a_data[nx] * tan(theta1);
      } else {
        XieMianPianYi_data[nx] = 0.0;
      }
    } else if ((double)nx + 1.0 <= 3.0 * ab1) {
      if (XMFlagA_data[nx] == 1.0) {
        /* 斜面上 */
        XieMianPianYi_data[nx] = a_data[nx] * tan(theta2 - 1.5707963267948966);
      } else {
        XieMianPianYi_data[nx] = 0.0;
      }
    } else if (XMFlagA_data[nx] == 1.0) {
      /* 斜面上 */
      XieMianPianYi_data[nx] = a_data[nx] * tan(theta3);
    } else {
      XieMianPianYi_data[nx] = 0.0;
    }
  }
  ...
  for (b_i = 0; b_i < loop_ub; b_i++) {
    if ((double)b_i + 1.0 <= ab1) {
      if (XMFlagA_data[b_i] == 1.0) {
        ...
        deltTib_data[b_i] = fabs(Ti_data[b_i] - ab4) * h / 2.0 *
                                tan(theta4 - 1.5707963267948966) -
                            XieMianPianYi_data[b_i] * sin(theta4);
      }
    } else if ((double)b_i + 1.0 <= 2.0 * ab1) {
      if (XMFlagA_data[b_i] == 1.0) {
        /* 斜面上 */
        ...
        deltTib_data[b_i] =
            fabs(Ti_data[b_i] - TiYuZhi1) * h / 2.0 * tan(theta1) -
            XieMianPianYi_data[b_i] * sin(theta1);
      }
    } else if ((double)b_i + 1.0 <= 3.0 * ab1) {
      if (XMFlagA_data[b_i] == 1.0) {
        /* 斜面上 */
        /*  deltTib(k) = deltTih(k).*tan(theta2); */
        deltTib_data[b_i] = fabs(Ti_data[b_i] - TiYuZhi2) * h / 2.0 *
                                tan(theta2 - 1.5707963267948966) +
                            XieMianPianYi_data[b_i] * sin(theta2);
      }
    } else if (XMFlagA_data[b_i] == 1.0) {
      /* 斜面上 */
      /*  deltTib(k) = deltTih(k).*tan(theta3); */
      deltTib_data[b_i] =
          fabs(Ti_data[b_i] - TiYuZhi3) * h / 2.0 * tan(theta3) +
          XieMianPianYi_data[b_i] * sin(theta3);
    }
  }
  i = deltTih->size[0] * deltTih->size[1];
  deltTih->size[0] = 1;
  deltTih->size[1] = deltTib->size[1];
  emxEnsureCapacity_real_T(deltTih, i);
  deltTih_data = deltTih->data;
  nx = deltTib->size[1];
  for (i = 0; i < nx; i++) {
    deltTih_data[i] = deltTib_data[i];
  }
  nx = deltTib->size[1];
  i = deltTib->size[0] * deltTib->size[1];
  deltTib->size[0] = 1;
  deltTib->size[1] = deltTih->size[1];
  emxEnsureCapacity_real_T(deltTib, i);
  deltTib_data = deltTib->data;
  for (b_i = 0; b_i < nx; b_i++) {
    deltTib_data[b_i] = fabs(deltTih_data[b_i]);
  }
  /*  4、计算高h向移动 */
  i = deltTih->size[0] * deltTih->size[1];
  deltTih->size[0] = 1;
  deltTih->size[1] = loop_ub;
  emxEnsureCapacity_real_T(deltTih, i);
  deltTih_data = deltTih->data;
  for (i = 0; i < loop_ub; i++) {
    deltTih_data[i] = 0.0;
  ...
  for (b_i = 0; b_i < loop_ub; b_i++) {
    if ((double)b_i + 1.0 <= ab1) {
      if (XMFlagA_data[b_i] == 1.0) {
        /* 斜面上 */
        deltTih_data[b_i] =
            -XieMianPianYi_data[b_i] * cos(theta4 - 1.5707963267948966);
      } else {
        deltTih_data[b_i] = 0.0;
      }
    } else if ((double)b_i + 1.0 <= 2.0 * ab1) {
      if (XMFlagA_data[b_i] == 1.0) {
        /* 斜面上 */
        deltTih_data[b_i] = XieMianPianYi_data[b_i] * cos(theta1);
      } else {
        deltTih_data[b_i] = 0.0;
      }
    } else if ((double)b_i + 1.0 <= 3.0 * ab1) {
      if (XMFlagA_data[b_i] == 1.0) {
        /* 斜面上 */
        deltTih_data[b_i] =
            XieMianPianYi_data[b_i] * cos(theta2 - 1.5707963267948966);
      } else {
        deltTih_data[b_i] = 0.0;
      }
    } else if (XMFlagA_data[b_i] == 1.0) {
      /* 斜面上 */
      deltTih_data[b_i] = -XieMianPianYi_data[b_i] * cos(theta3);
    } else {
      deltTih_data[b_i] = 0.0;
    }
  }
  ...
  i = (int)shenglunum;
  for (nx = 0; nx < i; nx++) {
    ab4 = Ti_data[nx] * h / 2.0 + deltTih_data[nx];
    ab2 = b / 2.0 - deltTib_data[nx];
    PointTable_A_off8_data[3 * nx] = (Pout[0] + u[0] * ab4) + v[0] * ab2;
    PointTable_A_off8_data[3 * nx + 1] = (Pout[1] + u[1] * ab4) + v[1] * ab2;
    PointTable_A_off8_data[3 * nx + 2] = (Pout[2] + u[2] * ab4) + v[2] * ab2;
  }
  i1 = (int)(2.0 * shenglunum + (1.0 - (shenglunum + 1.0)));
  for (b_i = 0; b_i < i1; b_i++) {
    ab1 = (shenglunum + 1.0) + (double)b_i;
    ab4 = Ti_data[(int)ab1 - 1] * h / 2.0 + deltTih_data[(int)ab1 - 1];
    ab2 = b / 2.0 - deltTib_data[(int)ab1 - 1];
    i2 = 3 * ((int)ab1 - 1);
    PointTable_A_off8_data[i2] = (Pout[0] + u[0] * ab4) - v[0] * ab2;
    PointTable_A_off8_data[i2 + 1] = (Pout[1] + u[1] * ab4) - v[1] * ab2;
    PointTable_A_off8_data[i2 + 2] = (Pout[2] + u[2] * ab4) - v[2] * ab2;
  }
  emxFree_real_T(&deltTib);
  /*  6、计算轴线h向移动l */
  /* 整体移动 */
  foot_of_perpendicular_from_a_point_to_a_line(PAB, Pin, Pout, &xN1, &ab2,
                                               &ab4);
  ab1 = ab4 - Pout[2];
  ab4 = ab2 - Pout[1];
  ab2 = xN1 - Pout[0];
  ab4 = sqrt((ab1 * ab1 + ab4 * ab4) + ab2 * ab2);
  /*  距离*cos(phi) 无正负 全为正 */
  ...
  for (b_i = 0; b_i < loop_ub; b_i++) {
    /*  tdb = deltTib(i); */
    /* %%%%%%  !!!! 不需要斜面偏移转到轴向  %%%%%%%%%%%%%%%%%%% */
    XMFlagA_data[b_i] = b / 2.0 / tan(phi);
  }
  /*  7、偏移 */
  /*  A测点 */
  i2 = deltTih->size[0] * deltTih->size[1];
  deltTih->size[0] = 1;
  deltTih->size[1] = loop_ub;
  emxEnsureCapacity_real_T(deltTih, i2);
  deltTih_data = deltTih->data;
  for (i2 = 0; i2 < loop_ub; i2++) {
    deltTih_data[i2] = 0.0;
  }
  for (b_i = 0; b_i < i; b_i++) {
    ...
    ab2 = tan(phi);
    for (i2 = 0; i2 < nx; i2++) {
      deltTih_data[i2] = (ab4 + XMFlagA_data[i2]) - a_data[i2] / ab2;
    }
  ...
  for (b_i = 0; b_i < i1; b_i++) {
    ...
    if (nx == i6 - i5) {
      for (i2 = 0; i2 < nx; i2++) {
        deltTih_data[i7 + i2] =
            (ab4 - XMFlagA_data[i3 + i2]) + a_data[i5 + i2] / x;
      }
    } else {
      binary_expand_op_10(deltTih, i7, ab4, XMFlagA, i3, i4 - 1, a, i5, i6 - 1,
                          x);
      deltTih_data = deltTih->data;
    }
    /*  PYA(1+shenglunum:2*shenglunum) = ztyd -
     * PhiPy(1+shenglunum:2*shenglunum); */
  }
  if (PointTable_A_off8->size[1] == deltTih->size[1]) {
    ...
    for (i2 = 0; i2 < nx; i2++) {
      XieMianPianYi_data[3 * i2] =
          PointTable_A_off8_data[3 * i2] - d[0] * deltTih_data[i2];
      i3 = 3 * i2 + 1;
      XieMianPianYi_data[i3] =
          PointTable_A_off8_data[i3] - d[1] * deltTih_data[i2];
      i3 = 3 * i2 + 2;
      XieMianPianYi_data[i3] =
          PointTable_A_off8_data[i3] - d[2] * deltTih_data[i2];
    }
  } else {
    binary_expand_op_14(PointTable_B_off8, PointTable_A_off8, d, deltTih);
  }
  /*  B测点 */
  i2 = deltTih->size[0] * deltTih->size[1];
  deltTih->size[0] = 1;
  ...
  for (b_i = 0; b_i < i1; b_i++) {
    ...
    if (nx == i11 - i10) {
      for (i2 = 0; i2 < nx; i2++) {
        deltTih_data[i12 + i2] =
            (ab4 + XMFlagA_data[i8 + i2]) - a_data[i10 + i2] / b_x;
      }
    } else {
      binary_expand_op_12(deltTih, i12, ab4, XMFlagA, i8, i9 - 1, a, i10,
                          i11 - 1, b_x);
      deltTih_data = deltTih->data;
    }
  }
  ...
  if (PointTable_A_off8->size[1] == deltTih->size[1]) {
    ...
    binary_expand_op_13(PointTable_A_off8, d, deltTih);
  }
  emxFree_real_T(&b_side_faces_transformed2);
  emxFree_real_T(&deltTih);
  /*  8、编号 */
  ...
  for (b_i = 0; b_i < i; b_i++) {
    ab2 = 2.0 * ((double)b_i + 1.0);
    XieMianPianYi_data[2 * b_i] = ab2 - 1.0;
    XieMianPianYi_data[2 * b_i + 1] = ab2;
  }
  for (b_i = 0; b_i < i1; b_i++) {
    ab4 = (shenglunum + 1.0) + (double)b_i;
    ab2 = ((2.0 * shenglunum + 1.0) - ab4) * 2.0;
    i = 2 * ((int)ab4 - 1);
    XieMianPianYi_data[i] = ab2;
    XieMianPianYi_data[i + 1] = ab2 - 1.0;
  }
}
