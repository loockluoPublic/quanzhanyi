                                           ...
                                           emxArray_real_T *PointTable_A_off)
{
  ...
  for (itilerow = 0; itilerow < 3; itilerow++) {
    ...
    for (boffset = 0; boffset < 3; boffset++) {
      P2D[itilerow + 3 * boffset] =
          (d * rot1[3 * boffset] + AngOring * rot1[3 * boffset + 1]) +
          norm_vec * rot1[3 * boffset + 2];
    }
  }
  C[0] = P2D[0];
  C[1] = P2D[3];
  ...
  for (boffset = 0; boffset < ibmat; boffset++) {
    AngProcess_data[(ntilerows + boffset) - 1] =
        (AngOring + 3.1415926535897931) + Ang_data[itilerow + boffset];
  }
  emxInit_real_T(&x, 2);
  /*  调用计算坐标 */
  /*  x = zeros(1,length(Ang)); */
  ...
  for (itilerow = 0; itilerow < ibmat; itilerow++) {
    x_data[itilerow] = AngProcess_data[itilerow];
  }
  ibmat = AngProcess->size[1];
  for (ntilerows = 0; ntilerows < ibmat; ntilerows++) {
    x_data[ntilerows] = cos(x_data[ntilerows]);
  }
  itilerow = x->size[0] * x->size[1];
  x->size[0] = 1;
  emxEnsureCapacity_real_T(x, itilerow);
  ...
  if (x->size[1] == AngProcess->size[1]) {
    ...
    ibmat = x->size[1];
    for (itilerow = 0; itilerow < ibmat; itilerow++) {
      Ang_data[itilerow] = x_data[itilerow];
    }
    ibmat = AngProcess->size[1];
    for (itilerow = 0; itilerow < ibmat; itilerow++) {
  ...
  x_data = b_PointTable2DT_A->data;
  for (itilerow = 0; itilerow <= ibmat; itilerow++) {
    x_data[itilerow] = Ang_data[itilerow + PointTable2DT_A->size[0]];
  }
  for (itilerow = 0; itilerow <= ntilerows; itilerow++) {
    x_data[itilerow + b_PointTable2DT_A->size[0]] = -Ang_data[itilerow];
  }
  for (itilerow = 0; itilerow <= boffset; itilerow++) {
    x_data[itilerow + b_PointTable2DT_A->size[0] * 2] =
        Ang_data[itilerow + PointTable2DT_A->size[0] * 2];
  }
  itilerow = PointTable2DT_A->size[0] * PointTable2DT_A->size[1];
  ...
  for (boffset = 0; boffset < 3; boffset++) {
    ...
    for (itilerow = 0; itilerow < ntilerows; itilerow++) {
      x_data[ibmat + itilerow] = C[boffset];
    }
  }
  ...
  for (itilerow = 0; itilerow < ibmat; itilerow++) {
    Ang_data[3 * itilerow] = x_data[itilerow];
    Ang_data[3 * itilerow + 1] = x_data[itilerow + b_PointTable2DT_A->size[0]];
