...
%%%%%%%%%%%%%%%%%%%%%%  判断点是否在斜面上（利用阈值）  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
temp = shenglunum./2;
XMFlagB = zeros(1,2*shenglunum);

for i = 1:2*shenglunum
    TiiB  = abs(Ti(i));
    if i <= temp
        if abs(TiiB) < abs(TiYuZhi3)
            XMFlagB(i) = 0;
        else
            XMFlagB(i) = 1;
        end
    elseif i <= 2*temp
        if abs(TiiB) < abs(TiYuZhi2)
            XMFlagB(i) = 0;
        else
            XMFlagB(i) = 1;
        end
    elseif i <= 3*temp
        if abs(TiiB) < abs(TiYuZhi1)
            XMFlagB(i) = 0;
        else
            XMFlagB(i) = 1;
        end
    else
        if abs(TiiB) < abs(TiYuZhi4)
            XMFlagB(i) = 0;
        else
            XMFlagB(i) = 1;
        end
...
Ti2B = zeros(1,2*shenglunum);
XieMianPianYi = zeros(1,2*shenglunum);
%%%%%%%%%%%%%%%%%%%%%%  对斜面上的点进行偏移（修正Ti）  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
for j = 1:2*shenglunum
    if j <= temp
        if XMFlagB(j) == 1 %斜面上
            Ti2B(j) = Ti(j) + a(j) *sin(theta3);
            XieMianPianYi(j) = a(j) *tan(theta3);
        else
            Ti2B(j) = Ti(j);
            XieMianPianYi(j) = 0;
...
[PointTable_A_off4,PointTable_B_off4] = Calculat_JuXing_A_and_B_Points_after_Offest(Tao,UPP,Pin,b,h,PAB,phi,shenglunum,Ti2A,toff);
...
for k = 1:2*shenglunum
    if k <= temp
        if XMFlagA(k) == 1 %斜面上
            up2 =  PabLine + Pin2Pup.*h.*Ti2A(k)./2;
...
for l = 1:2*shenglunum
    if l <= temp
        if XMFlagB(l) == 1 %斜面上
            up2 =  PabLine + Pin2Pup.*h.*Ti2B(l)./2;
